...
with col1:
    with st.container(border=True):
        ...
        chart_ACERO = alt.Chart(df_ratio_GRAFICO[df_ratio_GRAFICO['Proyecto'].isin(selector_proyecto)]).mark_bar().encode(
            ...
            color=alt.Color('Categoría:N', scale=alt.Scale(domain=['ADITAMENTO', 'CASCO', 'PANGA','PROYECTO MEJORA','OPEX','BE CASCO'], range=['#2e86c1','#5dade2','#d6eaf8','#2e86c1','#5dade2']), legend=alt.Legend(title='Categoría',orient='bottom')),
...
with col2:
    with st.container(border=True):
        ...
        chart_Sold = alt.Chart(df_ratio_GRAFICO[df_ratio_GRAFICO['Proyecto'].isin(selector_proyecto)]).mark_bar().encode(
            ...
            color=alt.Color('Categoría:N', scale=alt.Scale(domain=['ADITAMENTO', 'CASCO', 'PANGA','PROYECTO MEJORA','OPEX','BE CASCO'], range=['#FFC300','#FF5733','#C70039','#FFC300','#FF5733']), legend=alt.Legend(title='Categoría',orient='bottom')),
...
with col1:
    with st.container(border=True):
        ...
        chart_oxigeno = alt.Chart(df_ratio_GRAFICO[df_ratio_GRAFICO['Proyecto'].isin(selector_proyecto)]).mark_bar().encode(
            ...
            color=alt.Color('Categoría:N', scale=alt.Scale(domain=['ADITAMENTO', 'CASCO', 'PANGA','PROYECTO MEJORA','OPEX','BE CASCO', range=['#82e0aa ','#abebc6','#d5f5e3','#82e0aa ','#abebc6']), legend=alt.Legend(title='Categoría',orient='bottom')),
...
with col2:
    with st.container(border=True):
        ...
        chart_Sold = alt.Chart(df_ratio_GRAFICO[df_ratio_GRAFICO['Proyecto'].isin(selector_proyecto)]).mark_bar().encode(
            ...
            color=alt.Color('Categoría:N', scale=alt.Scale(domain=['ADITAMENTO', 'CASCO', 'PANGA','PROYECTO MEJORA','OPEX','BE CASCO'], range=['#85929e','#d6dbdf','#aeb6bf','#85929e','#d6dbdf']), legend=alt.Legend(title='Categoría',orient='bottom')),
